...
export const getDropPosition = (event: React.DragEvent): DropPosition => {
  ...
  if (horizontal < edgeThreshold) return "left";
  if (horizontal > 1 - edgeThreshold) return "right";
  if (vertical < edgeThreshold) return "top";
  if (vertical > 1 - edgeThreshold) return "bottom";
...
const splitLeaf = (
  ...
  const first = position === "left" || position === "top" ? newLeaf : branch;
  const second = position === "left" || position === "top" ? branch : newLeaf;
